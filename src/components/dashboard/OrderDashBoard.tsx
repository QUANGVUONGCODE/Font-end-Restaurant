...
const statusBadge = (status?: string) => {
  ...
  if (s.includes('complete') || s.includes('done') || s.includes('hoàn')) return 'bg-emerald-100 text-emerald-700';
  if (s.includes('pending') || s.includes('chờ') || s.includes('đợi')) return 'bg-amber-100 text-amber-800';
  if (s.includes('cancel') || s.includes('hủy')) return 'bg-red-100 text-red-700';
...
const OrderDashboardPage: React.FC = () => {
    ...
    try {
      ...
      if (data.code === 0) {
        const mapped: OrderRow[] = (data?.result?.orders || []).map((order: any) => ({
          ...
          numberOfGuest: order?.number_of_guest ?? 0,
          note: order?.note ?? '',
  ...
  const fetchOrderDetails2 = async (orderId: number) => {
    ...
    try {
      ...
      if (data.code === 0 && Array.isArray(data.result)) {
        const orderDetail = {
          orderCode: data.result[0]?.order?.orderCode,
          fullName: data.result[0]?.order?.user?.fullName,
          phoneNumber: data.result[0]?.order?.user?.phoneNumber,
          numberOfGuest: data.result[0]?.order?.numberOfGuest,
          tableName: data.result[0]?.order?.table?.name,
          orderDate: data.result[0]?.order?.orderDate,
          totalMoney: data.result[0]?.order?.totalMoney,
          status: data.result[0]?.order?.status,
          note: data.result[0]?.order?.note,
  ...
  const handleCompleteOrder = async (id: number) => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/restaurant/api/v1/order/status/${id}`, {
        method: 'PUT',
        headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
      });
      const data = await response.json();
      if (data.code === 0) {
        ...
        toast.error('❌ Không thể hoàn thành đơn');
      }
    } catch (error) {
      console.error('Error completing order:', error);
      toast.error('Lỗi khi hoàn thành đơn');
    }
  };
      ...
      <div className="p-6 md:p-8 ml-64">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-gray-600">Tìm kiếm, xem chi tiết, hoàn thành & chỉnh sửa đơn</p>
          </div>
                  ...
                  orders.map((order) => (
                    <tr key={order.id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-4 py-3">
                        ...
                        <div className="text-xs text-gray-500">Bàn: {order.tableName} • {order.numberOfGuest} khách</div>
                      </td>
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="relative inline-block text-left">
                          ...
                          {dropdownOpen === order.id && (
                            <div className="absolute right-0 mt-2 w-44 bg-white border border-gray-100 shadow-xl rounded-xl overflow-hidden z-20">
                              <button
                                onClick={() => {
                              ...
                              <button
                                ...
                                className="w-full px-4 py-2.5 text-left text-gray-800 hover:bg-gray-50 flex items-center gap-2"
                              >
                                <CheckCircle2 className="w-4 h-4 text-emerald-600" /> Hoàn thành
                              </button>
            ...
            <div className="w-full max-w-lg bg-white rounded-2xl shadow-2xl overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={selectedOrder.number_of_guest || ''}
        ...
        <div className="fixed inset-0 z-50">
          ...
          <div className="absolute inset-0 flex items-center justify-center p-4">
            <div className="w-full max-w-4xl bg-white rounded-2xl shadow-2xl overflow-hidden">
              <div className="px-6 py-4 border-b flex items-center justify-between bg-gradient-to-r from-blue-600 to-teal-500 text-white">
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="bg-gray-50 border rounded-2xl p-4">
                        ...
                        <div className="font-semibold text-gray-900">{orderDetails2.orderCode || '—'}</div>
                      ...
                      <div className="bg-gray-50 border rounded-2xl p-4 space-y-2">
                        <div className="flex items-center gap-2 text-gray-800">
                          ...
                          <span className="font-medium">{orderDetails2.fullName || '—'}</span>
                        </div>
                        <div className="flex items-center gap-2 text-gray-800">
                          <Phone className="w-4 h-4 text-gray-500" />
                          <span>{orderDetails2.phoneNumber || '—'}</span>
                        </div>
                      </div>
                      ...
                      <div className="bg-gray-50 border rounded-2xl p-4 space-y-2">
                        <div className="flex items-center gap-2 text-gray-800">
                          ...
                          <span>Bàn: <b>{orderDetails2.tableName || '—'}</b></span>
                        </div>
                        <div className="flex items-center gap-2 text-gray-800">
                          <Users2 className="w-4 h-4 text-gray-500" />
                          <span>Số khách: <b>{orderDetails2.numberOfGuest ?? '—'}</b></span>
                        </div>
                        <div className="flex items-center gap-2 text-gray-800">
                          <CalendarDays className="w-4 h-4 text-gray-500" />
                          <span>{orderDetails2.orderDate || '—'}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-5 grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="md:col-span-2 bg-white border rounded-2xl p-4">
                        ...
                        <div className="text-gray-900 mt-1">{orderDetails2.note || 'Không có ghi chú'}</div>
                      </div>
                      <div className="bg-gradient-to-r from-emerald-500 to-green-600 text-white rounded-2xl p-4">
                        ...
                        <div className="text-2xl font-bold mt-1">
                          {formatMoney(orderDetails2.totalMoney)}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="mt-6">
                      ...
                      <div className="space-y-3">
                        {orderDetails2.foodDetails?.map((item: any, idx: number) => (
                          <div
                            key={idx}
                            className="flex flex-col sm:flex-row sm:items-center gap-4 bg-gray-50 border rounded-2xl p-4"
                          >
                            ...
                            <div className="flex-1">
                              <div className="font-semibold text-gray-900">{item.foodName || '—'}</div>
