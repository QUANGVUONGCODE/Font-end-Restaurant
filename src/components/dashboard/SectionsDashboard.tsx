...
const SectionDashboardPage: React.FC = () => {
  ...
  const fetchSections = async () => {
    ...
    try {
      ...
      if (data.code === 0) {
        setSections(Array.isArray(data.result) ? data.result : []);
      } else {
  ...
  const filtered = useMemo(() => {
    ...
    return sections.filter(
      (s) => s.name.toLowerCase().includes(key) || String(s.id).includes(key)
    );
  }, [sections, q]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (formMode === 'create') {
        ...
        const res = await fetch(`http://localhost:8080/restaurant/api/v1/sections/${editingId}`, {
          method: 'PUT',
          headers: {
            'Accept-Language': 'vi',
            'Content-Type': 'application/json',
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`http://localhost:8080/restaurant/api/v1/sections/${deletingId}`, {
        method: 'DELETE',
        headers: {
          'Accept-Language': 'vi',
          Authorization: `Bearer ${token}`,
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50">
      <div className="p-6 md:p-8 ml-64">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">Quản lý Section</h2>
            <p className="text-sm text-gray-600">Thêm, sửa, xóa section</p>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-lg overflow-hidden">
          <div className="px-6 py-4 border-b bg-gradient-to-r from-blue-500 to-teal-400 text-white flex items-center justify-between">
            <span className="font-semibold">Danh sách section</span>
            <span className="text-sm text-white/90">{filtered.length} section</span>
          </div>
          ...
          <table className="min-w-full table-auto">
            <thead className="bg-gray-50 text-gray-700">
              <tr>
                ...
                <th className="px-4 py-3 text-left">Tên section</th>
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={3} className="px-6 py-10 text-center text-gray-600">
                    Không có section nào phù hợp.
                  </td>
                </tr>
              ) : (
                filtered.map((s) => (
                  <tr key={s.id} className="hover:bg-gray-50 transition">
                    <td className="px-4 py-3 text-center font-medium text-gray-900">{s.id}</td>
                    <td className="px-4 py-3 text-left text-gray-800">{s.name}</td>
                    <td className="px-4 py-3">
            ...
            <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
              <div className="px-6 py-4 border-b flex items-center justify-between bg-gradient-to-r from-blue-500 to-purple-600 text-white">
                <div className="font-semibold">
                  {formMode === 'create' ? 'Thêm section' : 'Chỉnh sửa section'}
                </div>
              ...
              <div className="p-6">
                <label className="text-sm text-gray-600">Tên section</label>
                <input
                  value={nameInput}
            ...
            <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
              ...
              <div className="p-6">
                <p className="text-gray-800">
                  Bạn chắc chắn muốn xóa section:
